...
List *create_list(size_t datasize, int capacity) {
    ...
    sem_init(&(list->semaphore), 0, 1); /* Semaphore başlatıldığı kısım */
...
Node *add(List *list, void *data) {
    sem_wait(&(list->semaphore)); /* Semaphore kilitlenmesi */

    Node *node = NULL;

    if (list->number_of_elements >= list->capacity) {
        perror("list is full!");
        sem_post(&(list->semaphore)); /* Semaphore kilidi açılıyor */
        return NULL;
    }
    ...
    sem_post(&(list->semaphore)); /* Semaphore kilidinin açılması */
